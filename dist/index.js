var React = require('react');

function _extends() {
  _extends = Object.assign || function (target) {
    for (var i = 1; i < arguments.length; i++) {
      var source = arguments[i];

      for (var key in source) {
        if (Object.prototype.hasOwnProperty.call(source, key)) {
          target[key] = source[key];
        }
      }
    }

    return target;
  };

  return _extends.apply(this, arguments);
}

function Avto(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M13.9 14.15h1.5c.3 0 .6-.3.6-.6v-.8m-9.8 1.4H4.7c-.3 0-.6-.3-.6-.6v-.8m0-2.8h11.8m-11.8 0H2.7c-.4 0-.7-.3-.7-.7 0-.4.3-.7.7-.7h2.1l-.7 1.4zm11.8 0h1.4c.4 0 .7-.3.7-.7 0-.4-.3-.7-.7-.7h-2.1l.7 1.4zm1.8 2.4-2.1-2.8-1.3-3.5c-.2-.5-.7-.9-1.3-.9-2.1-.2-4-.2-5.9 0-.6 0-1.1.3-1.3.9l-1.4 3.6-2.1 2.7c-.2.3-.3.6-.3.9v4.4c0 .4.3.7.7.7h.7c.4 0 .7-.3.7-.7v-1.4h11.8v1.4c0 .4.3.7.7.7h.7c.4 0 .7-.3.7-.7v-4.4c0-.3-.1-.6-.3-.9z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Bivak(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M3.895 10.59H2.853c-.474 0-.853-.404-.853-.909C2 5.436 5.505 2 9.863 2c4.358 0 7.863 3.436 7.863 7.681 0 .505-.379.91-.852.91h-1.137m0-.405c0-3.335-2.653-6.064-5.874-6.064-3.22 0-5.874 2.73-5.874 6.064v6.772c0 .505.38 1.01.948 1.01h9.947c.474 0 .948-.404.948-1.01v-6.772h-.095zm-8.053 7.782h4.169v-3.941c0-1.213-.853-2.122-1.99-2.122h-.095c-1.136 0-1.99.91-1.99 2.122v3.941h-.094zM8.821 6.952h1.895c.663 0 1.137.506 1.137 1.213v.101c0 .708-.474 1.213-1.137 1.213H8.82c-.568 0-1.137-.607-1.137-1.213v-.101c0-.707.569-1.213 1.137-1.213z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Cerkev(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M10 2v3.333m0 0L6.19 9.43M10 5.333l3.81 4.096M8.476 3.524h3.048m1.714 5.333V18m0 0H6.762m6.476 0h4.19v-5M6.763 18V8.857m0 9.143h-4.19v-5M2 12.762l4.762-2.095M18 12.762l-4.762-2.095M11.524 18H8.476v-2.667c0-.857.667-1.524 1.524-1.524s1.524.667 1.524 1.524V18zm0-8.19a1.524 1.524 0 1 1-3.048 0 1.524 1.524 0 0 1 3.048 0z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function CestnoKolo(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m15.2 12.1-3.5-3.4-1.3 3m-5.8.7L7.1 7H4.8l.3-1m7.6 2h-2m-.5 3.9L6.6 8.2M2 15.7h15.8m-14.9 2h2.9m3 0h2.9m2.9 0h2.9m.3-5.4a2.6 2.6 0 1 1-5.2 0 2.6 2.6 0 0 1 5.2 0zm-10.6 0a2.6 2.6 0 1 1-5.2 0 2.6 2.6 0 0 1 5.2 0z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Drevesa(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m13.632 6.178 2.987 3.873h-6.082m.32-4.178L9.15 7.809m-2.882 7.134C3.921 14.943 2 13.108 2 10.866S3.92 6.79 6.268 6.79s4.269 1.834 4.269 4.076c0 1.835-1.83 3.363-3.537 3.873h10.9l-3.201-4.382m-1.494 4.79V18H11.39v-2.854M6.268 9.848V18m0-4.28 1.708-1.835M4.56 11.07 6.268 12.6M12.138 2 9.15 5.873h5.869l-1.921-2.65L12.138 2z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Drsanje(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M13.782 15.718c2.536 0 2.863-.736 2.863-.736 0-.9-.245-3.11-2.863-3.6C11.164 10.89 9.118 8.764 9.118 5H3.391v4.664c0 1.309-1.718 2.454-.655 4.582m11.046 1.472h-2.127c-2.864 0-1.637-1.473-4.337-1.473m6.464 1.473v2.128m-6.464-3.6v1.39H2.736v-1.39m4.582 0c0-2.128-1.882-3.273-4.009-3.191m-.573 3.19h4.337M2 17.846h12.845c1.391 0 2.455-.736 2.864-1.39l-1.39-1.228m-4.01.491v2.128M5.6 15.718v2.128m-1.473-2.128v2.128M9.282 6.964l-1.555.572m2.128.982-1.473.737m1.063 1.472 1.228-1.063m.245 2.127.573-1.31m-8.1-4.09h3.273",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Dvorec(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M4.2 13.8v.9m11.7-.9v.9m-7.7 2.5v-1.6s-.1-2 1.8-2c1.9 0 1.8 2 1.8 2v1.3M8.8 9v1m2.6-1v1m-5.2.8H2v6.7h16v-6.7h-4.2V6.7H6.2v4.1zM15 6.7 10 2 4.9 6.7H15z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Ferata(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m18.3 17.376-.301.4a.5.5 0 0 0 .801-.4h-.5zm-3.622-2.735-.497-.053a.5.5 0 0 0 .195.452l.302-.4zm.572-5.377.497.053a.5.5 0 0 0-.041-.258l-.456.205zm-1.907-4.245.456-.205a.5.5 0 0 0-.259-.255l-.197.46zm-3.527-1.51-.298-.401a.5.5 0 0 0 .102.861l.196-.46zm1.907-1.415-.007-.5a.5.5 0 0 0-.291.099l.298.401zM18.3 2h.5a.5.5 0 0 0-.507-.5l.007.5zm-5.338 11.763a.5.5 0 1 0 0 1v-1zm1.525 1a.5.5 0 1 0 0-1v1zm-.953-3.924a.5.5 0 1 0 0 1v-1zm.953 1a.5.5 0 1 0 0-1v1zm-.953-3.358a.5.5 0 1 0 0 1v-1zm.953 1a.5.5 0 0 0 0-1v1zm-1.906-3.264a.5.5 0 1 0 0 1v-1zm1.048 1a.5.5 0 1 0 0-1v1zm4.354 10.608a.5.5 0 0 0 .443-.897l-.443.896zM2.06 17.445a.5.5 0 0 0-.122.993l.122-.993zm7.51-4.314-.5.017.5-.017zM8.099 8.503l-.388.315.388-.315zm-3.154 4.63.5.018-.5-.019zm0 .603h.5-.5zm1.918 3.723a.5.5 0 1 0 1 0h-1zm1-5.333a.5.5 0 1 0-1 0h1zm-4.785 4.93a.5.5 0 1 0 1 0h-1zm1-1.106a.5.5 0 0 0-1 0h1zm.41-3.246a.5.5 0 0 0 .914-.408l-.913.408zM4 10.516l.346-.361-.346.36zm-.841 0-.346-.361.346.36zM2.002 14l-.5-.013.5.013zm3.367 1.31a.5.5 0 1 0-.847-.532l.847.532zM18.6 16.977l-3.622-2.735-.603.798L18 17.775l.602-.798zm-3.426-2.283.572-5.377-.995-.106-.571 5.377.994.106zm.53-5.635L13.8 4.814l-.912.41 1.907 4.245.912-.41zm-2.165-4.5-3.527-1.51-.393.92 3.527 1.51.393-.92zm-3.426-.648 1.907-1.415-.596-.803-1.907 1.415.596.803zm1.616-1.317 6.577-.094-.014-1-6.577.094.014 1zM17.8 2v15.376h1V2h-1zm-4.838 12.763h1.525v-1h-1.525v1zm.572-2.924h.953v-1h-.953v1zm0-2.358h.953v-1h-.953v1zm-.953-2.264h1.048v-1h-1.048v1zm5.845 9.711a1.059 1.059 0 0 0-.245-.078 4.247 4.247 0 0 0-.26-.043 17.09 17.09 0 0 0-.729-.083 84.065 84.065 0 0 0-2.074-.165c-1.566-.109-3.139-.198-3.837-.247l-.07.997c.636.045 2.353.144 3.837.248a82.77 82.77 0 0 1 2.048.162c.28.027.513.053.684.078.087.012.151.023.196.032.06.013.046.015.007-.005l.443-.896zm-7.145-.616c-.41-.03-1.03.069-1.707.197-.673.128-1.555.318-2.403.483-1.77.343-3.674.632-5.11.454l-.122.993c1.614.2 3.665-.125 5.423-.466.915-.177 1.69-.347 2.399-.481.706-.134 1.193-.201 1.45-.183l.07-.997zm-1.21-3.197c-.035-1.02-.32-2.107-.639-2.992-.313-.868-.688-1.618-.944-1.935l-.777.63c.146.18.475.797.78 1.645.3.832.55 1.81.58 2.686l1-.034zM8.488 8.188c-.19-.233-.534-.688-1.125-.688v1c.02 0 .043.002.096.043.066.052.138.134.252.275l.777-.63zM7.363 7.5c-.584 0-.918.41-1.137.705l.803.597c.105-.143.176-.22.233-.263.043-.033.068-.039.101-.039v-1zm-1.137.705c-.142.19-.562.906-.954 1.803-.391.898-.787 2.05-.826 3.105l.999.038c.032-.867.368-1.881.744-2.743.377-.863.761-1.501.84-1.606l-.803-.597zm-1.78 4.908c-.01.246 0 .502 0 .623h1c0-.179-.01-.36-.001-.585l-1-.038zm0 .623c0 1.551 1.33 2.613 2.812 2.613v-1c-1.04 0-1.813-.718-1.813-1.613h-1zm2.812 2.613c1.502 0 2.813-1.195 2.813-2.613h-1c0 .796-.791 1.613-1.813 1.613v1zm2.813-2.613c0-.208.008-.376 0-.62l-1 .033c.008.227 0 .323 0 .587h1zm-2.208 3.723v-5.333h-1v5.333h1zm-3.785-.402V15.95h-1v1.107h1zm.867-4.557.457-.204v-.001l-.003-.005-.007-.015a31.382 31.382 0 0 0-.124-.277c-.081-.179-.19-.419-.303-.662-.112-.243-.229-.492-.325-.688a7.649 7.649 0 0 0-.188-.356c-.009-.015-.048-.081-.107-.137l-.692.722c-.045-.044-.066-.084-.051-.06l.028.05c.028.053.067.128.112.221.091.186.204.427.316.667a86.715 86.715 0 0 1 .421.928l.007.016.002.004.457-.203zm-.6-2.345c-.114-.11-.345-.34-.766-.34v1h.01a.042.042 0 0 1-.01-.004s.005.002.017.013c.014.012.03.026.057.053l.692-.723zm-.766-.34c-.422 0-.653.23-.767.34l.692.722.057-.053.017-.013a.042.042 0 0 1-.01.004h.01v-1zm-.767.34c-.053.05-.091.11-.102.127-.02.03-.042.067-.064.104a6.08 6.08 0 0 0-.154.29c-.117.232-.26.543-.402.897-.277.694-.567 1.607-.588 2.414l1 .026c.016-.626.252-1.405.517-2.07.13-.325.262-.61.366-.817a5.03 5.03 0 0 1 .161-.296c.016-.024-.002.008-.042.047l-.692-.723zm-1.31 3.832c-.004.14 0 .273 0 .353h1c0-.122-.004-.201 0-.327l-1-.026zm0 .353c0 1.101.98 2.01 2.077 2.01v-1c-.586 0-1.077-.502-1.077-1.01h-1zm2.077 2.01c.4 0 .755-.098 1.064-.29.305-.189.54-.453.726-.75l-.847-.532a1.3 1.3 0 0 1-.405.432.985.985 0 0 1-.538.14v1z",
    fill: "currentColor"
  }));
}

function GorskoKolo(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m15.783 12.331-4.629-1.337m4.629 1.235-2.469-4.115-1.954 2.572m-5.657-.823L9.406 5.44l-2.16-.617L7.76 4M2 11.714l2.571.926 2.778 2.263 3.497-.514 4.217.514 3.497 2.88m-4.114-9.977-1.852-.515m-1.44 3.6L8.686 6.47m.72 10.285 2.365-.205m-8.845-1.955 1.028.72m14.297-2.88a2.571 2.571 0 1 1-5.142 0 2.571 2.571 0 0 1 5.142 0zM8.377 9.863a2.571 2.571 0 1 1-5.143 0 2.571 2.571 0 0 1 5.143 0z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Grad(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M5.19 9.07H2m3.19 0s0 3.628.1 3.628c0-.186 9.473 0 9.473 0V9.07m-9.572 0V7.488M2 9.07V18h15.954V9.07M2 9.07V7.488M17.954 9.07h-3.19m3.19 0V7.488m-3.19 1.582V7.488m-4.787-3.72 2.991 3.441H6.686l3.291-3.442zm0 0V2h1.695v.93h1.895m2.792 4.558v1.303M3.596 7.488v1.303m6.381.186v.93m-6.381 1.86v.93m1.595-.93v6.047m9.572-6.047v6.047m1.596-6.047v.93m-8.177 5.117v-1.488s-.1-1.86 1.795-1.86 1.795 1.86 1.795 1.86v1.209M7.883 7.302h3.989v5.024H7.883V7.302z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Hrib(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m6.2 11.8.7 1.4 1.4-2.1 1.4 2.1 1.4-2.1 1.4 2.1.9-1.7M9.7 6.2 2 18h16L9.7 6.2zm0 0V2m4.2 2.8H9.7V2.7h4.2l-.7 1.1.7 1z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Izvir(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M11.036 9.594a.5.5 0 1 0-.407.913l.407-.913zm1.373 1.16-.204.457.026.012.026.008.152-.476zm-.105 1.51.087.493.033-.006.032-.01-.152-.477zM1.915 13.583a.5.5 0 0 0 .173.985l-.173-.985zm9.158-4.078a.5.5 0 1 0-.143.99l.143-.99zm5.857 1.35.179-.467-.052-.02L17 10.36l-.071.495zm.42 2.415.265.425.014-.009.013-.01-.291-.405zm-7.834 4.306a.5.5 0 1 0 .53.848l-.53-.848zm.055-10.444-.5.017.5-.017zM8.099 2.503l-.388.315.388-.315zm-3.154 4.63.5.018-.5-.019zm0 .603h.5-.5zm1.918 3.723a.5.5 0 1 0 1 0h-1zm1-5.333a.5.5 0 1 0-1 0h1zm-4.785 4.93a.5.5 0 1 0 1 0h-1zm1-1.106a.5.5 0 0 0-1 0h1zm.41-3.246a.5.5 0 0 0 .914-.408l-.913.408zM4 4.516l.346-.361-.346.36zm-.841 0-.346-.361.346.36zM2.002 8l-.5-.013.5.013zm3.367 1.31a.5.5 0 1 0-.847-.532l.847.532zm2.551 7.849a.5.5 0 0 0 .358-.934l-.358.934zm-.872-1.473.443-.232a.5.5 0 0 0-.873-.025l.43.256zm-2.313-.101.45-.22a.5.5 0 0 0-.742-.186l.292.406zm-1.472.707a.5.5 0 1 0 0 1v-1zm8.772-.99a.5.5 0 1 0 .538-.843l-.538.843zm-.572-1.327.443-.232a.5.5 0 0 0-.9.027l.457.205zm-1.02.439a.5.5 0 1 0 .358.934l-.358-.934zm.186-3.907 1.576.704.408-.913-1.577-.704-.407.913zm1.628.724c.075.024.178.106.178.329h1c0-.583-.317-1.104-.874-1.282l-.304.953zm.178.329c0-.021.006.013-.061.085a.556.556 0 0 1-.222.143l.304.953c.242-.078.472-.226.646-.411.17-.18.333-.448.333-.77h-1zm-.217.212L1.915 13.583l.173.985 10.303-1.811-.173-.985zm-1.288-1.277 5.928.855.143-.99-5.928-.855-.143.99zm5.821.827c.418.16.68.453.74.743l.98-.205c-.15-.716-.728-1.229-1.362-1.472l-.358.934zm.74.743c.05.23-.095.557-.432.8l.583.812c.504-.362.99-1.042.829-1.817l-.98.205zm-.405.781-7.57 4.73.53.848 7.57-4.73-.53-.848zm-7.015-5.73c-.035-1.02-.32-2.108-.639-2.993-.313-.868-.688-1.618-.944-1.935l-.777.63c.146.18.475.797.78 1.645.3.832.55 1.81.58 2.686l1-.034zM8.488 2.187c-.19-.233-.534-.688-1.125-.688v1c.02 0 .043.002.096.043.066.052.138.134.252.275l.777-.63zM7.363 1.5c-.584 0-.918.41-1.137.705l.803.597c.105-.143.176-.22.233-.263.043-.033.068-.039.101-.039v-1zm-1.137.705c-.142.19-.562.906-.954 1.803-.391.898-.787 2.05-.826 3.105l.999.038c.032-.867.368-1.881.744-2.743.377-.863.761-1.501.84-1.606l-.803-.597zm-1.78 4.908c-.01.246 0 .502 0 .623h1c0-.179-.01-.36-.001-.585l-1-.038zm0 .623c0 1.551 1.33 2.613 2.812 2.613v-1c-1.04 0-1.813-.718-1.813-1.613h-1zm2.812 2.613c1.502 0 2.813-1.195 2.813-2.613h-1c0 .796-.791 1.613-1.813 1.613v1zm2.813-2.613c0-.208.008-.376 0-.62l-1 .033c.008.227 0 .323 0 .587h1zm-2.208 3.723V6.126h-1v5.333h1zm-3.785-.402V9.95h-1v1.107h1zM4.945 6.5l.457-.204v-.001l-.003-.004a24.641 24.641 0 0 1-.034-.077 98.897 98.897 0 0 0-.4-.878c-.112-.243-.229-.492-.325-.688a7.674 7.674 0 0 0-.13-.256 2.06 2.06 0 0 0-.058-.1c-.009-.015-.048-.081-.107-.137l-.692.722c-.045-.044-.066-.084-.051-.06l.028.05c.028.053.067.128.112.222.091.185.204.426.316.666a86.808 86.808 0 0 1 .395.869l.026.06.007.015.002.004.457-.203zm-.6-2.345c-.114-.11-.345-.34-.766-.34v1h.01a.046.046 0 0 1-.01-.004s.005.002.017.013c.014.012.03.026.057.053l.692-.722zm-.766-.34c-.422 0-.653.23-.767.34l.692.722.057-.053.017-.013a.046.046 0 0 1-.01.004h.01v-1zm-.767.34c-.053.05-.091.11-.102.127-.02.03-.042.067-.064.104a6.045 6.045 0 0 0-.154.29c-.117.232-.26.543-.402.897-.277.694-.567 1.607-.588 2.414l1 .026c.016-.626.252-1.405.517-2.07.13-.325.262-.61.366-.817a5.08 5.08 0 0 1 .161-.296c.016-.024-.002.009-.042.047l-.692-.722zm-1.31 3.832c-.004.14 0 .273 0 .353h1c0-.122-.004-.201 0-.327l-1-.026zm0 .353c0 1.101.98 2.01 2.077 2.01v-1c-.586 0-1.077-.502-1.077-1.01h-1zm2.077 2.01c.4 0 .755-.098 1.064-.29.305-.189.54-.453.726-.75l-.847-.532a1.3 1.3 0 0 1-.405.432.986.986 0 0 1-.538.139v1zm4.699 5.875c-.368-.141-.607-.426-.787-.771l-.886.463c.24.46.632.98 1.315 1.242l.358-.934zm-1.66-.796c-.197.33-.37.546-.513.672-.142.125-.217.13-.237.129-.018 0-.09-.01-.222-.144-.133-.136-.289-.366-.462-.72l-.898.438c.195.4.407.737.645.98.24.247.542.433.904.445.362.012.675-.153.93-.377.254-.223.489-.536.712-.91l-.859-.513zm-2.175-.25a2.822 2.822 0 0 0-.41.384c-.136.148-.232.262-.358.395a2.068 2.068 0 0 1-.303.277c-.09.061-.12.058-.109.058v1c.275 0 .506-.117.677-.235.173-.12.33-.273.462-.413.124-.13.27-.3.368-.406.116-.127.197-.206.257-.248l-.584-.812zm8.13-.72a1.916 1.916 0 0 1-.667-.716l-.886.463c.257.493.64.856 1.015 1.096l.538-.843zm-1.566-.688a1.423 1.423 0 0 1-.397.532.985.985 0 0 1-.173.113l-.002.001.003-.001.002-.001h.002v-.001h.001l.18.466.178.467h.002l.002-.001.005-.002.011-.005a1.458 1.458 0 0 0 .13-.063c.077-.042.177-.106.288-.196a2.42 2.42 0 0 0 .68-.9l-.912-.41z",
    fill: "currentColor"
  }));
}

function Jama(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M7.612 2.929 10.613 2l3.095 1.548.75 2.994 1.5 2.684.376 3.51s.656 1.238 1.219 2.477c.562 1.239.75 2.787-.376 2.787H2.736c-1.313 0-.469-4.129-.469-4.129l2.157-2.58s-.657-2.375-.375-3.717C4.33 6.232 5.924 5.82 5.924 5.82l1.688-2.89zm0 0 .844 1.342 1.126.723m5.814 4.438-1.5.103-.845-.412M2.55 14.077l1.406.826 1.97-.62M8.175 18v-5.058c0-1.445 1.125-2.684 2.438-2.684 1.313 0 2.438 1.239 2.438 2.684V18c-.656 0-4.876-.103-4.876 0z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Jezero(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M13.822 4.65v6.522m0-3.873 1.325-1.12m-2.854-.714 1.529 1.121m1.12 9.07c-.407-.102-.713-.408-.917-.815-.713 1.223-2.344 1.223-3.057 0-.713 1.223-2.344 1.223-3.057 0-.306.611-.917.917-1.529.917M17.49 5.567a3.567 3.567 0 1 1-7.133 0 3.567 3.567 0 0 1 7.133 0zm.816 9.783c0 1.464-3.65 2.65-8.153 2.65S2 16.814 2 15.35c0-1.463 3.65-2.65 8.153-2.65s8.153 1.187 8.153 2.65z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Kajak(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M11.853 13.616h.947c1.61 0 3.505-.405 4.358-2.02.095-.203.19-.405.19-.607 0-.202-.159-.489-.348-.489h-3.126m-.695-1.127-3.63 5.253m0 0 .57 1.313-.854 1.515c-.284.505-1.042.707-1.515.404-.474-.303-.663-.91-.38-1.414l.853-1.515 1.327-.303zm2.588-1.01H5.789c-1.894 0-2.652-.405-3.6-2.02-.094-.203-.189-.405-.189-.708 0-.202.19-.404.379-.404h12.884m-9.19-.303 1.611-3.636c.284-.707 1.042-1.01 1.8-.808 0 0 .569.202 1.137.707l.474.808c.19.202.379.404.663.606l1.42.606c.38.202.57.606.475.91m-1.99.504s-1.61-.909-1.99-1.414m2.653-4.949C12.326 2.707 11.758 2 11 2c-.758 0-1.421.606-1.421 1.414 0 .808.568 1.515 1.326 1.515.758 0 1.421-.606 1.421-1.414z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Kamping(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M4.1 13.744 2 18v-1.68m2.1-2.576L10 5.12l5.9 8.624m-11.8 0V18h4.2v-3.136c0-1.12.8-2.128 1.7-2.128.5 0 .9.224 1.2.56.3.336.5.896.5 1.456v3.136h4.2v-4.144m0 0L18 18v-1.68m-13.9-.448H8m3.7 0h3.8M10.8 6.24 9.3 4m.3 1.68L10.7 4",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Koca(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M16 9H4v9h12V9.08M4 12h12M4 15h4m4 0h4M2 10.312 10 2l8 8.312M6.5 6h7M4.556 7.463V5M8 14h4v4H8v-4z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Kopalisce(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M2 16.7c1.232 0 1.232 1.3 2.558 1.3 1.231 0 1.231-1.3 2.558-1.3 1.231 0 1.231 1.3 2.558 1.3 1.231 0 1.231-1.3 2.558-1.3 1.231 0 1.231 1.3 2.558 1.3 1.231 0 1.231-1.3 2.557-1.3M9.295 5.5V4.1c0-1.2-.853-2.1-1.99-2.1s-1.99.9-1.99 2.1v11.1m11.843-9.7V4.1c0-1.2-.853-2.1-1.99-2.1-1.136 0-1.99.9-1.99 2.1v11.1M6 8h7m-7 4h7",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function KopanjePsov(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M17.047 17.753c-.734-.275-1.285-.734-1.652-1.376-1.192 1.927-4.037 1.927-5.23 0-1.192 1.927-4.036 1.927-5.229 0a3.04 3.04 0 0 1-2.569 1.468m0-3.578c1.101 0 2.11-.55 2.57-1.468 1.192 1.926 4.036 1.926 5.229 0 1.192 1.926 4.037 1.926 5.23 0 .275.55.825 1.009 1.376 1.192m-12.845-.275c.183-.459-.092-.459-.092-1.193-.092-.825.734-1.743 1.468-2.018.734-.275 1.743-.184 2.385-.092.643 0 2.478.184 3.12.184.55 0 .917-.276 1.376-.826.459-.55.55-1.285.642-1.652.092-.367.367-.734.367-.734s-.367-.55-.458-1.009c-.184-.826.55-1.376.55-1.376.367.184.734.917.734.917.275.276.184.826.642 1.01.459.183.826.917 1.101 1.1.367.276 2.019.092 2.202.551.184.459-.367.734-1.009 1.193-.642.459-1.468-.092-1.927.275-.459.367-.55 2.66-.55 3.12m-2.386-3.303s.367.825 1.01 1.1c.55.276 1.467.368 1.467.368m-9.633-.826s-1.468-.826-2.019-1.468C2.184 8.21 2 7.294 2 6.927",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Piknik(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M7.5 11.9 3.3 18m9.1-6.1 4.2 6.1M4.4 14.5h11.1M2 11.3h16m-11-.6c-1.1 0-2-.9-2-2V7.5h3.9v1.2c0 1.2-.9 2-1.9 2zm3.9-2h3.2v2.1h-3.2V8.7z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Pin(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M15.8 7.9c0 3.5-4 8.1-5.4 9.6-.3.3-.7.3-1 0C8 16 4 11.4 4 7.9 4 4.7 6.7 2 9.9 2s5.9 2.6 5.9 5.9z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }), /*#__PURE__*/React.createElement("path", {
    d: "M9.8 10.8a2.8 2.8 0 1 0 0-5.6 2.8 2.8 0 0 0 0 5.6z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Planina(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M18 9.79v6.105C18 17.053 17.1 18 16 18H4c-1.1 0-2-.947-2-2.105v-3.158m15.5 3.684s-3.7-3.79-7.8-4c-2.6-.21-6-.631-7.5.21M8.8 12s2.4-4.632 5.9-4.316c2.5.316 3.2 2 3.2 2m-12.2 2.21V6.317m2.2 1.052L6.4 2.526C6.4 2.211 6.1 2 5.7 2c-.3 0-.6.21-.7.526L3.6 7.368c-.1.21-.1.421-.1.632 0 1.263 1 2.21 2.2 2.21 1.2 0 2.2-1.052 2.2-2.21.1-.21 0-.421 0-.632z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Plavanje(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M18.527 18.173c-.82-.307-1.436-.82-1.846-1.538-1.333 2.153-4.511 2.153-5.844 0-1.333 2.153-4.511 2.153-5.844 0a3.398 3.398 0 0 1-2.871 1.64m0-3.998c1.23 0 2.358-.615 2.87-1.64 1.334 2.153 4.512 2.153 5.845 0 1.333 2.153 4.511 2.153 5.844 0a2.795 2.795 0 0 0 1.538 1.332m-6.254-.41 2.665-2.665a.805.805 0 0 0 0-1.128l-3.178-2.769-4.306-2.46-4.306 1.23c-.616.103-.923.718-.82 1.333.204.615.82.923 1.332.82l3.281-.923L9.607 8.74l-4.102 4.307m11.452-9.325A2.46 2.46 0 1 1 13.475 7.2a2.46 2.46 0 0 1 3.482-3.478z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function PlezanjeFerata(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m14.063 15.89-3.435-2.23a.436.436 0 0 0-.502 0l-2.597 1.168c-.168.106-.419 0-.503-.319l-.837-3.292 2.01-.744c.168-.106.335-.212.335-.425L9.54 7.607c.168-.425.084-1.063-.335-1.275-.335-.319-.754-.106-.921.319l-.67 1.7c-.084.105-.252.318-.335.318l-1.508.53-2.346 2.019 1.34 5.417c.252 1.062 1.09 1.593 1.927 1.274l3.853-1.593c.168-.106.336 0 .503 0l2.262 1.487M9.623 7.606l4.607 5.735 2.43-.743L18 15.147M9 6 6.116 4.476 5 2m5 1H8m9 6h-4M4.824 7.5c0 .828-.633 1.5-1.412 1.5C2.632 9 2 8.328 2 7.5S2.632 6 3.412 6c.78 0 1.412.672 1.412 1.5z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Pohodnistvo(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M8.17 5.942c-.192-.385-.673-.577-1.057-.673H6.92c-.865 0-1.538.673-1.634 1.442-.096.673-.192.865-.385 1.058-.48.288-.865 1.153-.865 1.826v.385c-.096.48 0 .961.288 1.25a1.37 1.37 0 0 0 1.154.576l.673.193h.289m6.633-5.384L15.19 17.96m-1.635-7.692s-3.269-.096-4.326-.673m5-1.057c-.29-.289-.578-.385-1.058-.385h-1.827c-.192 0-.385-.096-.577-.192-.48-.577-.77-1.827-1.827-1.827h-.096c-.769 0-1.346.577-1.442 1.25l-.673 4.615c0 .384-.384 2.5-.384 2.5l-.673 1.826c-.289.48-.577 1.346-.097 1.635.481.288 1.25-.289 1.539-.577l1.153-2.115s.193-2.02.289-2.116l1.442 1.731.865 2.404c.096.384.385.673 1.154.673.384 0 .577-.577.48-.962l-.576-3.557c-.096-.289-2.308-2.211-2.308-2.211s.385-.673.481-1.058m2.02-6.538a1.634 1.634 0 1 1-3.27 0 1.634 1.634 0 0 1 3.27 0z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function RazgledniStolp(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M4 18 5.857 7.235m7.54 0L15.253 18H4.109m.984-5.136 9.942 4.84m-1.092-4.84-9.724 4.84m1.748-9.185h7.32M11.211 2v5.827M5.967 9.111l7.976 3.95m-.984-3.95-7.757 3.95M15.8 4.766h-4.589V2.691H15.8l-.765 1.087.765.987z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Ribolov(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M13.9 5.876c-.5.199-1 .298-1.6.298-.9 0-1.7-.298-2.4-.696m1.972 10.215L10.2 18h4.3l-1.772-2.307m-3.71-7.865L7 11.448h2m6.562-3.62 2.138 3.62h-2M3.9 9.254v6.062c0 .994.8 1.789 1.8 1.789 2.6 0 2.1-3.876 0-3.876l.2.994m-2-4.97c.497 0 .9-.4.9-.894a.897.897 0 0 0-.9-.894c-.497 0-.9.4-.9.894s.403.895.9.895zm0-1.888V4.087m11.8 5.068c0 3.975-3.4 6.957-3.4 6.957S8.9 13.13 8.9 9.155c0-1.391.4-2.683.9-3.776C10.8 3.292 12.2 2 12.2 2s1.4 1.391 2.4 3.379c.7 1.192 1.1 2.484 1.1 3.776z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Roza(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M10 18c1.4-4.9 2.8-7 7-7-2.8 4.2-2.8 7-7 7zm0 0c-1.4-4.9-2.8-7-7-7 2.8 4.2 2.8 7 7 7zm1.4-14.6c0 .8-1.4 2.8-1.4 2.8s-1.4-2-1.4-2.8c0-.8.6-1.4 1.4-1.4.8 0 1.4.6 1.4 1.4zM6.5 5.5c.8 0 2.8 1.4 2.8 1.4s-2 1.4-2.8 1.4c-.8 0-1.4-.6-1.4-1.4 0-.8.6-1.4 1.4-1.4zm2.1 4.9c0-.8 1.4-2.8 1.4-2.8s1.4 2 1.4 2.8c0 .8-.6 1.4-1.4 1.4-.8 0-1.4-.7-1.4-1.4zm4.9-2.1c-.8 0-2.8-1.4-2.8-1.4s2-1.4 2.8-1.4c.8 0 1.4.6 1.4 1.4 0 .8-.7 1.4-1.4 1.4z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function SkakanjeVVodo(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M17.254 17.915c-.686-.255-1.2-.681-1.544-1.277-1.114 1.787-3.772 1.787-4.887 0-1.114 1.787-3.772 1.787-4.887 0A2.846 2.846 0 0 1 3.536 18m5.144-.68v-2.554H3.107m5.23-.34v-3.064c0-.17-.086-.34-.171-.51L6.108 9.83a.41.41 0 0 1 0-.596L8.08 7.277l1.543 1.532c.086.085.257.17.343.17h2.23a.766.766 0 1 0 0-1.532h-1.458c-.172 0-.258-.085-.429-.17L9.28 6.17 6.708 4.723 3.45 7.957c-.6.596-.6 1.617 0 2.213l3 2.043c.172.085.172.34.172.51v1.788m4.116-11.064c0 .799-.653 1.447-1.458 1.447a1.452 1.452 0 0 1-1.457-1.447c0-.8.652-1.447 1.457-1.447s1.458.648 1.458 1.447z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Slap(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M7.884 2.004H18m-10.22 0s-1.98-.152-2.28 1.5c-.82 4.5.532 10.644-.5 12m11-13.5s-2.1 0-2.5 1c-1 2.5.661 10.99-.5 12.5M7.4 5.392v4.472m1.826 3.388v1.355m1.858-7.183.103 5.15M2 18.004c.5-.356 1-1 2-1s2.5 1 4 1 2.688-1 4-1c1.313 0 2 1 3 1s2-1 3-1M2.548 13.658l.207.678m13.561-1.22-.206.678",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Soteska(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M17.798 14.73s-1.418-.467-3.14-.467c-1.316 0-2.936.817-4.253.934-1.924.117-2.734-1.985-5.266-1.168-1.012.35-2.329 1.168-2.531 1.168M17.898 18s-1.214-.584-2.733-.817c-1.317-.234-3.545.467-4.76.583-1.924.117-3.24-1.167-5.772-.467-1.013.35-1.823.467-2.025.35M2 12.629 6.557 2l3.038 6.657 2.33-3.504L18 11.343m-4.76-8.76h1.114m2.228 2.804h-1.114M4.835 6.555 5.95 8.073l2.43-1.635",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Sup(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m3 6 10.491 7.31m.135-11.051c-2.04-1.145-5.537 1.673-8.063 5.197-2.525 3.523-4.371 8.808-3.205 9.513.485.264.777.44 1.262.704 1.36.793 6.12-3.523 8.258-6.518 2.234-3.259 3.691-7.751 1.748-8.896zm-3.594 8.192c-1.749 2.202-4.954 5.197-5.634 4.844-.68-.352 1.165-4.492 2.331-6.43.971-1.497 2.137-2.73 3.109-3.523 0-.088 2.137 1.321 2.428 1.497 0 0-1.457 2.643-2.234 3.612zm3.557 4.357 1.942 1.498c.486.352 1.263.352 1.652-.177l.194-.176c.389-.44.389-1.145-.194-1.497l-1.943-1.497-1.749.176.098 1.673z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Supanje(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M11.181 14.363v-3.426c0-.149-.074-.298-.148-.447L9.245 8.852a.36.36 0 0 1 0-.522l1.639-1.713 1.34 1.341a.569.569 0 0 0 .373.149h1.936m0 0c.372 0 .67-.298.67-.67a.667.667 0 0 0-.67-.67h-1.266a.57.57 0 0 1-.373-.15l-.968-.968-2.16-1.266-2.83 2.83a1.394 1.394 0 0 0 0 1.937l2.607 2.532a.569.569 0 0 1 .149.372v2.309m4.84-6.256L12.3 14.959M3 15h14M13.192 3.266a1.266 1.266 0 1 1-2.532 0 1.266 1.266 0 0 1 2.532 0zm-2.01 12.512-.447 1.266c-.15.372.074.82.447.968h.149c.372.15.819-.074.968-.447l.372-1.266-.521-.968-.969.447z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Trdnjava(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M12.6 2v5.9m-2.8 4.7h-.9m3.5 2.3h-.8m-5 .6h-.9M16.8 4.8h-4.2V2.7h4.2l-.7 1.1.7 1zM12.5 18H5.1C3.9 18 3 17.3 3 16.5c0 0 1-8 1-8.1L5.6 8v1.3h1.9V8H10v1.3h1.7V8c.6 0 1.5.4 1.5.4l1.4 8.1c.1.8-.9 1.5-2.1 1.5z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Voda(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M18 16.896a3.666 3.666 0 0 1-1.766-1.556c-1.247 2.179-4.468 2.179-5.715 0-1.246 2.179-4.467 2.179-5.714 0C4.182 16.377 3.143 17 2 17m16-4.774a3.666 3.666 0 0 1-1.766-1.556c-1.247 2.18-4.468 2.18-5.715 0-1.246 2.18-4.467 2.18-5.714 0-.623 1.037-1.662 1.66-2.805 1.66m16-4.773A3.666 3.666 0 0 1 16.234 6c-1.247 2.18-4.468 2.18-5.715 0-1.246 2.18-4.467 2.18-5.714 0C4.182 7.038 3.143 7.66 2 7.66",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Zastava(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M11.7 10.3c.5 0 1-.5 1-1s-.4-1-1-1h-1m1.1-6.2c.5 0 1 .4 1 1V9m-1.1-7H3v6.3m0 0h7.7M3 8.3V18m7.7-9.7v2.8c0 .8.6 1.4 1.4 1.4H17l-1.4-2.8L17 6.9h-4.2",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Zemljevid(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M15.55 9.7s-.112 0-.112.113c-.339.113-.79.339-1.242.903M2 6.312v11.406l4.856-1.694 5.985 1.58 5.194-1.919V5.522l-5.194 1.694-3.275-1.13m-.339 7.68c-.226 0-.339-.114-.564-.114-.34-.113-.678-.225-1.13-.451l1.694.564zm2.259 0c.113 0 .338-.114.451-.227.565-.338.904-.79 1.13-1.242l-1.581 1.468zM5.614 12.07C4.71 9.022 2 8.345 2 5.635 2 3.715 3.694 2.02 5.614 2.02s3.613 1.694 3.613 3.614c0 2.71-2.71 3.387-3.613 6.436zm0-7.678c-.678 0-1.243.564-1.243 1.242 0 .677.565 1.242 1.243 1.242.677-.113 1.242-.565 1.242-1.242 0-.678-.565-1.242-1.242-1.242z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Zicnica(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "M10.259 9.306v7.2M2 4.965 18.518 2m-6.883 2.541c0 .741-.635 1.377-1.376 1.377A1.402 1.402 0 0 1 8.882 4.54c0-.741.636-1.376 1.377-1.376m0 4.235V6.024M5.918 14.6h3.388m1.906 0h3.282m.953-7.2 2.859 4.765-1.906 6.247H4.012l-1.906-6.247L4.965 7.4h10.482zm-9.953 9.106-1.27-4.13 1.905-3.07h8.26l1.905 3.07-1.27 4.13h-9.53z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function Zoga(props) {
  return /*#__PURE__*/React.createElement("svg", _extends({
    width: "1em",
    height: "1em",
    viewBox: "0 0 20 20",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    strokeLinecap: "round",
    strokeLinejoin: "round"
  }, props), /*#__PURE__*/React.createElement("path", {
    d: "m11.2 3.7 5.5 5.5s-2.5 4.9-7.5 7.5l-5.6-5.6c2.6-5 7.5-7.5 7.5-7.5m1.8 1.8c-2.8 1.6-6.7 6-7.5 7.5m9.3-5.8c-1.6 2.8-6 6.7-7.5 7.5m3.7-10.9V2m7.4 6.9-1.7.2M3.6 11l-1.5.1M9 18.3l.1-1.7M7.8 5.9l-1.3-3m-.9 5.2L2.8 6.6m14.7 7.3-2.9-1.2m-.7 4.9-1.5-2.8m6-4.6a8.2 8.2 0 1 1-16.4 0 8.2 8.2 0 0 1 16.4 0z",
    stroke: "currentColor",
    strokeMiterlimit: 10
  }));
}

function useIcons() {
  var poiTypesIcons = React.useMemo(function () {
    return {
      jezero: Jezero,
      slap: Slap,
      koca: Koca,
      bivak: Bivak,
      cerkev: Cerkev,
      drugo: Pin,
      dvorec: Dvorec,
      ferata: Ferata,
      grad: Grad,
      hrib: Hrib,
      izvir: Izvir,
      jama: Jama,
      kopalisce: Kopalisce,
      'muzej-na-prostem': Drevesa,
      planina: Planina,
      'razgledni-stolp': RazgledniStolp,
      soteska: Soteska,
      'sup-tocka': Sup
    };
  }, []);
  var interestsIcons = React.useMemo(function () {
    return {
      pohodnistvo: Pohodnistvo,
      kultura: Zastava,
      'poletna-osvezitev': Voda,
      feratanje: PlezanjeFerata,
      'naravne-lepote': Roza,
      raziskovanje: Zemljevid,
      supanje: Supanje
    };
  }, []);
  return {
    poiTypesIcons: poiTypesIcons,
    interestsIcons: interestsIcons
  };
}

exports.Avto = Avto;
exports.Bivak = Bivak;
exports.Cerkev = Cerkev;
exports.CestnoKolo = CestnoKolo;
exports.Drevesa = Drevesa;
exports.Drsanje = Drsanje;
exports.Dvorec = Dvorec;
exports.Ferata = Ferata;
exports.GorskoKolo = GorskoKolo;
exports.Grad = Grad;
exports.Hrib = Hrib;
exports.Izvir = Izvir;
exports.Jama = Jama;
exports.Jezero = Jezero;
exports.Kajak = Kajak;
exports.Kamping = Kamping;
exports.Koca = Koca;
exports.Kopalisce = Kopalisce;
exports.KopanjePsov = KopanjePsov;
exports.Piknik = Piknik;
exports.Pin = Pin;
exports.Planina = Planina;
exports.Plavanje = Plavanje;
exports.PlezanjeFerata = PlezanjeFerata;
exports.Pohodnistvo = Pohodnistvo;
exports.RazgledniStolp = RazgledniStolp;
exports.Ribolov = Ribolov;
exports.Roza = Roza;
exports.SkakanjeVVodo = SkakanjeVVodo;
exports.Slap = Slap;
exports.Soteska = Soteska;
exports.Sup = Sup;
exports.Supanje = Supanje;
exports.Trdnjava = Trdnjava;
exports.Voda = Voda;
exports.Zastava = Zastava;
exports.Zemljevid = Zemljevid;
exports.Zicnica = Zicnica;
exports.Zoga = Zoga;
exports.useIcons = useIcons;
//# sourceMappingURL=index.js.map
